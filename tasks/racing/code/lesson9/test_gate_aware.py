...
from gate_aware_env import (DEV_SEEDS, EPISODE_TIME, FLOOR_PENALTY, FLOOR_Z, GATE_PENALTY, MISS_WINDOW,  # noqa: E402
                            POOL_SPEC, POOL_TRAJS, GateAwareContrastEnv, GateAwareForceContrastEnv, GateAwareTrackingEnv,
                            gate_contacts, gate_progress,
                            summarise_gate_stats,
                            train_seeds)
...
# ---- the saved recipe, and the contrast group built on the same gate logic ---------------------------------------
def _load_saved_v4():
    """The gate_aware_env.py that trained the saved v4 checkpoint (saved/gate_aware_v4/code/), executed as if it sat
    in the live directory: its `__file__` is pointed at the live file so its track-pool discovery sees the same
    `tracks/` dir (loading it from saved/ would silently give it an empty train pool)."""
    import types

    here = Path(__file__).resolve().parent
    src = (here / "saved" / "gate_aware_v4" / "code" / "gate_aware_env.py").read_text()
    mod = types.ModuleType("gate_aware_env_v4_saved")
    mod.__file__ = str(here / "gate_aware_env.py")
    exec(compile(src, mod.__file__, "exec"), mod.__dict__)
    return mod


def test_refactored_robust_env_is_bit_identical_to_the_saved_v4_recipe():
    """The mixin refactor (needed so the contrast group shares the gate logic instead of copying it) must not change
    the saved recipe: same seed, same random actions, 500 steps over 8 envs (random actions crash quickly, so many
    episodes end and reset -> `_sample_traj`, RNG use, stats are all exercised). Exact equality, every step."""
    old = _load_saved_v4()
    assert len(old.POOL_TRAJS) == len(POOL_TRAJS) and old.EPISODE_TIME == EPISODE_TIME
    a = old.GateAwareTrackingEnv(num_envs=8, seed=0, v3=True, v5=True)
    b = GateAwareTrackingEnv(num_envs=8, seed=0, v3=True, v5=True)
    oa, _ = a.reset()
    ob, _ = b.reset()
    ...
    resets = 0
    for k in range(500):
        act = rng.uniform(-1, 1, size=(8, 4)).astype(np.float32)
        ra, rb = a.step(act), b.step(act)
        for x, y, name in zip(ra[:4], rb[:4], ("obs", "reward", "crashed", "truncated")):
            assert np.array_equal(x, y), f"step {k}: {name} differs"
        resets += int(ra[2].sum() + ra[3].sum())
    assert a.stats == b.stats and a.stats["episodes"] > 8, a.stats
    assert np.array_equal(a._dev_idx, b._dev_idx) and np.array_equal(a._gate_idx, b._gate_idx)
    print(f"  identical over 500 steps, {resets} episode ends, stats {b.stats}")


def test_refactored_robust_env_is_bit_identical_under_the_trained_policy():
    """Random actions end episodes almost only on the floor/divergence branch. Driving both envs with the SAVED v4
    policy (sampled, not deterministic) reaches gates, touches frames, misses gates and finishes laps -- the branches
    the refactor actually moved. Exact equality of obs/reward/terminated/truncated every step, and of the stats."""
    import torch
    from stable_baselines3 import PPO

    here = Path(__file__).resolve().parent
    model = PPO.load(str(here / "saved" / "gate_aware_v4" / "ckpt" / "datt_ppo_final.zip"), device="cpu")
    torch.manual_seed(0)
    old = _load_saved_v4()
    a = old.GateAwareTrackingEnv(num_envs=8, seed=3, v3=True, v5=True)
    b = GateAwareTrackingEnv(num_envs=8, seed=3, v3=True, v5=True)
    oa, _ = a.reset()
    b.reset()
    for k in range(1000):
        act, _ = model.predict(oa, deterministic=False)
        ra, rb = a.step(act), b.step(act)
        for x, y, name in zip(ra[:4], rb[:4], ("obs", "reward", "crashed", "truncated")):
            assert np.array_equal(x, y), f"step {k}: {name} differs"
        oa = ra[0]
    ...
    st = b.stats
    print(f"  identical over 1000 steps under the trained policy, stats {st}")
    assert st["episodes"] > 8 and st["contact"] + st["miss"] > 0 and st["gates_sum"] > 0, \
        f"the trained policy should reach gates in this rollout: {st}"


def _make_contrast(num_envs=4, seed=0):
    return GateAwareContrastEnv(num_envs=num_envs, seed=seed, v3=True, v5=True)


def test_contrast_group_shares_the_gate_logic_by_construction():
    assert GateAwareContrastEnv.step is GateAwareTrackingEnv.step
    assert GateAwareContrastEnv._sample_traj is GateAwareTrackingEnv._sample_traj
    assert GateAwareContrastEnv.__init__ is GateAwareTrackingEnv.__init__
    c, r = _make_contrast(), _make_env()
    assert c.freq == r.freq == 100 and c.max_steps == r.max_steps == int(EPISODE_TIME * 100)
...
def test_contrast_group_differs_only_in_disturbance_training():
    from crazy_track.envs.datt_env import PERTURB_ACC_MAX
    from crazy_track.sensors import LighthouseSensorBatch

    env = _make_contrast(num_envs=300)
    env.reset()
    acc = env.perturb_force / 0.04338
    assert acc.max() <= PERTURB_ACC_MAX + 1e-6 and acc.min() >= -PERTURB_ACC_MAX - 1e-6, "vendored +-3.5 box"
    assert acc[:, 0].max() > 0.8 * PERTURB_ACC_MAX and acc[:, 2].max() < 0.6 * PERTURB_ACC_MAX
    assert isinstance(env.sensor, LighthouseSensorBatch), "vendored noise-scale sensor, not the lambda-coupled one"
    assert not hasattr(env, "_mass_rng") and not hasattr(env, "dr_lam_max"), "the contrast group has no mass channel"
    np.testing.assert_allclose(np.asarray(env.sim.data.params.mass), np.asarray(env.sim.default_data.params.mass))


def test_contrast_group_terminates_on_a_miss_and_contact_like_the_robust_one():
    import jax.numpy as jnp

    env = _make_contrast(num_envs=3)
    env.reset()
    for i in range(3):
        env._traj[i] = POOL_TRAJS[1]; env._dev_idx[i] = 1; env._gate_idx[i] = 0
    env.steps[:] = 0
    env.steps[0] = int(round((POOL_TRAJS[1].gate_times[0] + MISS_WINDOW) * 100))
    _place(env, 0, POOL_TRAJS[1].pos((env.steps[0] + 1) / 100))
    _, reward, crashed, _, _ = env.step(np.zeros((3, 4), dtype=np.float32))
    assert crashed[0] and reward[0] == -GATE_PENALTY and env.stats["miss"] == 1
    assert not crashed[1] and not crashed[2]
    gate = POOL_TRAJS[0].gates[0]
    contact_pos = np.asarray(gate.center) + np.asarray(gate.rotation) @ np.array([0.0, 0.0, 0.28])
    env2 = _make_contrast(num_envs=2)
    env2.reset()
    for i in range(2):
        env2._traj[i] = POOL_TRAJS[0]; env2._dev_idx[i] = 0
    pos = np.array(env2.sim.data.states.pos); quat = np.array(env2.sim.data.states.quat)
    pos[0, 0] = contact_pos; quat[0, 0] = np.array([0.0, 0.0, 0.0, 1.0])
    env2.sim.data = env2.sim.data.replace(states=env2.sim.data.states.replace(pos=jnp.asarray(pos), quat=jnp.asarray(quat)))
    _, reward2, crashed2, _, _ = env2.step(np.zeros((2, 4), dtype=np.float32))
    assert crashed2[0] and reward2[0] == -GATE_PENALTY and env2.stats["contact"] == 1


def test_train_script_cli_selects_the_group_and_allows_the_studys_three_seeds():
    import train_gate_aware as tga

    a = tga.build_parser().parse_args(["--reason", "t"])
    assert (a.group, a.seed, a.timesteps, a.ckpt_every) == ("robust", 0, 16_000_000, 2_000_000)
    for g in ("robust", "contrast"):
        for sd in (0, 1, 2):
            args = tga.build_parser().parse_args(["--reason", "t", "--group", g, "--seed", str(sd)])
            assert (args.group, args.seed) == (g, sd)
    for bad in (["--seed", "3"], ["--group", "matched"]):
        try:
            tga.build_parser().parse_args(["--reason", "t"] + bad)
            raise AssertionError(f"{bad} should be rejected")
        except SystemExit:
            pass


def test_force_only_contrast_differs_from_robust_in_force_and_mass_only():
    from crazy_track.envs.datt_env import PERTURB_ACC_MAX
    from lighthouse_batch import LambdaLighthouseSensorBatch

    f = GateAwareForceContrastEnv(num_envs=300, seed=0, v3=True, v5=True)
    r = GateAwareTrackingEnv(num_envs=300, seed=0, v3=True, v5=True)
    f.reset(); r.reset()
    # same gate logic by construction, same sensing curriculum as the robust group
    assert GateAwareForceContrastEnv.step is GateAwareTrackingEnv.step
    assert isinstance(f.sensor, LambdaLighthouseSensorBatch) and isinstance(r.sensor, LambdaLighthouseSensorBatch)
    assert f.sensor.lam_max == r.sensor.lam_max == 0.8 and f.sensor.lam_eff.max() > 0
    assert abs(f.sensor.lam_eff.max() - r.sensor.lam_eff.max()) < 0.1 * r.sensor.lam_eff.max()
    # ... but the vendored force box and no mass channel
    acc = f.perturb_force / 0.04338
    assert acc.max() <= PERTURB_ACC_MAX + 1e-6 and acc.min() >= -PERTURB_ACC_MAX - 1e-6
    assert acc[:, 2].max() < 0.6 * PERTURB_ACC_MAX, "vendored z halving"
    assert np.abs(r.perturb_force / 0.04338).max() > PERTURB_ACC_MAX, "the robust box is wider than the vendored one"
    assert not hasattr(f, "_mass_rng")
    np.testing.assert_allclose(np.asarray(f.sim.data.params.mass), np.asarray(f.sim.default_data.params.mass))
    assert (np.asarray(r.sim.data.params.mass) > np.asarray(r.sim.default_data.params.mass) + 1e-9).any()
    # and it steps
    for _ in range(10):
        obs, rew, crashed, trunc, info = f.step(np.zeros((300, 4), dtype=np.float32))
    assert np.all(np.isfinite(obs)) and np.all(np.isfinite(rew))


def test_train_script_offers_the_force_only_contrast_group():
    import train_gate_aware as tga

    a = tga.build_parser().parse_args(["--reason", "t", "--group", "contrast_force", "--seed", "0"])
